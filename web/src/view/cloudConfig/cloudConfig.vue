<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="跑马灯大小" prop="transDrawtextFontsize" width="120" />
        ...
        <el-table-column align="left" label="滚动文字" prop="transDrawtextString" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateCloudConfigFunc(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getCloudConfigList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteCloudConfigByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateCloudConfigFunc = async(row) => {
    const res = await findCloudConfig({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteCloudConfigFunc = async (row) => {
    const res = await deleteCloudConfig({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
