<template>
  <div>
    <div class="gva-search-box">
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束时间"></el-date-picker>
        ...
        <el-form-item label="文件（夹）名">
         <el-input v-model="searchInfo.fileName" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="0根目录1一层目录2二层目录" prop="fileDegree" width="120" />
        <el-table-column align="left" label="1dir0文件" prop="fileIsDir" width="120" />
        <el-table-column align="left" label="最后一层dir名" prop="fileLastDir" width="120" />
        <el-table-column align="left" label="文件（夹）名" prop="fileName" width="120" />
        ...
        <el-table-column align="left" label="1video2other" prop="fileType" width="120" />
        <el-table-column align="left" label="更新者" prop="updatedBy" width="120" />
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateFileInputFunc(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getFileInputList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteFileInputByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateFileInputFunc = async(row) => {
    const res = await findFileInput({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.refileInput
...
// 删除行
const deleteFileInputFunc = async (row) => {
    const res = await deleteFileInput({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createFileInput(formData.value)
                  break
                case 'update':
                  res = await updateFileInput(formData.value)
                  break
                default:
                  res = await createFileInput(formData.value)
                  break
